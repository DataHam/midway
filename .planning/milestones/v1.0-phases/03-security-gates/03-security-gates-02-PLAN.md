...
files_modified:
  ...
  - tests/03-security-gates/verify-danny.api.test.js
autonomous: true
requirements:
  - GATE-04
  - GATE-05
...
must_haves:
  truths:
    - Valid token returns 302 redirect to danny.tamtham.com
  ...
  artifacts:
    ...
    - path: "tests/03-security-gates/verify-danny.api.test.js"
      provides: "Integration tests for Danny API"
...
<objective>
Implement Danny verification API endpoint with Turnstile token validation, rate limiting, and hybrid response format.

Purpose: Provide server-side validation for verify-danny.html gate page, protecting danny.tamtham.com subdomain access with Cloudflare Turnstile verification.
...
# Extracted from Plan 01 and RESEARCH.md — use these directly
...
```http
HTTP/1.1 302 Found
Location: https://danny.tamtham.com
```
...
{
  "success": true,
  "redirectUrl": "https://danny.tamtham.com"
}
```
...
<task type="auto" tdd="true">
  <name>Task 1: Implement Danny API endpoint with Turnstile validation</name>
  <files>src/api/verify-danny/index.js, tests/03-security-gates/verify-danny.api.test.js</files>
  <behavior>
  ...
  <action>
Create Danny verification API endpoint:
...
9. If validation fails:
   ...
   - Log failure event with error code
   - Increment rate limit counter
   - Return 403 with error JSON
10. If validation succeeds:
    - Clear rate limit counter
    - Log success event
    - Get redirect URL from query parameter or default to 'https://danny.tamtham.com'
...
Use existing Vitest structure for integration tests. Mock Azure Functions context and request objects.
  ...
  <verify>
    <automated>npm test -- tests/03-security-gates/verify-danny.api.test.js</automated>
  </verify>
  <done>
    - index.js exports Azure Functions HTTP handler
    - Handler validates Turnstile tokens correctly
    - Returns 302 redirect for browser requests
    ...
    - Logs anonymized IP and truncated user agent
...
1. src/api/verify-danny/index.js exists with handler implementation
2. src/api/verify-danny/function.json exists with correct configuration
3. tests/03-security-gates/verify-danny.api.test.js exists with integration tests
4. Running `npm test -- tests/03-security-gates/verify-danny.api.test.js` passes all tests
5. API returns correct status codes for all error scenarios
</verification>

<success_criteria>
- Valid token: Returns 302 redirect to danny.tamtham.com for HTML requests
- Valid token: Returns JSON { success: true, redirectUrl: 'https://danny.tamtham.com' } for API requests
...
- Logging: All events logged with anonymized IP, truncated user agent
- Tests: All integration tests pass
</success_criteria>

<output>
After completion, create `.planning/phases/03-security-gates/03-security-gates-02-SUMMARY.md`
</output>
