/**
 * Integration tests for Danny verification API endpoint
 */
...
import { handler } from '../../src/api/verify-danny/index.js';

describe('Danny Verification API', () => {
  let mockContext;
  ...
  const createMockRequest = (body = {}, headers = {}, searchParams = []) => {
    const mockReq = {
      ...
      json: async () => body,
      url: `http://localhost/api/verify-danny${searchParams.length > 0 ? '?' + searchParams.join('&') : ''}`
    };
    return mockReq;
  };
  ...
  describe('Test 1: Valid token returns 302 redirect for HTML requests', () => {
    it('should return 302 with Location header for browser requests', async () => {
      ...
      expect(response.headers.get('Location')).toBe('https://danny.tamtham.com');
  ...
  describe('Test 2: Valid token returns JSON with redirectUrl for API requests', () => {
    it('should return JSON with redirectUrl for API requests', async () => {
      ...
      expect(data.redirectUrl).toBe('https://danny.tamtham.com');
    });
  });
